...
async function reloadCorrections() {
  const res = await apiGet('/alertard/api/super_corrections_list.php');
...
async function openCorr(id) {
  currentCorrId = id;
  const data = await apiGet('/alertard/api/super_correction_detail.php?id=' + id);
  ...
  // Nombres legibles para FKs y valores numéricos donde aplique
  const html = `
  ...
  document.getElementById('btnApply').onclick = async ()=>{
    await apiPost('/alertard/api/super_correction_apply.php', {id: currentCorrId});
    modal.hide();
    await reloadCorrections();
  };
  document.getElementById('btnReject').onclick = async ()=>{
    await apiPost('/alertard/api/super_correction_reject.php', {id: currentCorrId});
    modal.hide();
    await reloadCorrections();
  };
}

// Llama a esto al cargar dashboard (además de loadPending())
reloadCorrections();
</script>
